...
export function PhotoSourceSheet({
  ...
  const closeSheet = useModalNavigation({ open, onBack: onClose });

  if (!open || typeof document === "undefined") return null;

  return createPortal(
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-[#173144]/35 backdrop-blur-[2px] sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="photo-source-title" onClick={() => closeSheet()}>
      <div className="w-full max-w-[440px] rounded-t-[28px] bg-[#eef9ff] px-5 pb-[max(24px,env(safe-area-inset-bottom))] pt-4 shadow-2xl sm:rounded-[28px] sm:pb-6" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="mt-3 grid grid-cols-[40px_1fr_40px] items-center">
          ...
          <button type="button" onClick={() => closeSheet()} className="icon-button" aria-label="닫기"><X size={19} /></button>
        </div>
        ...
        <button type="button" onClick={() => closeSheet()} className="mt-3 w-full rounded-2xl border-2 border-[#d2e3ec] py-3.5 text-sm font-extrabold text-[#718594]">취소</button>
      </div>
    </div>,
    document.body,
  );
