...
import type { CreateReportInput, Location, PrecipitationStatus, SunlightStatus, TemperatureStatus } from "@/lib/types";
...
export function ReportForm() {
  ...
  const mutation = useMutation({
    mutationFn: (values: ReportFormValues) => weatherApi.createReport({ ...values, location: reportLocation! } as CreateReportInput),
    onSuccess: (report) => {
    ...
    onError: () => showToast("제보를 올리지 못했어요.", "ERROR"),
  });
    ...
    <form onSubmit={submit} className="page">
      ...
      {step === 1 ? <>
        ...
        <button type="button" onClick={() => setNeedsManualInput(true)} className="flex w-full items-center gap-3 rounded-[20px] bg-white p-4 text-left shadow-sm shadow-[#b8d6e6]/20">
          ...
          <span><span className="block text-xs font-semibold text-[#718594]">현재 위치</span><span className="font-extrabold">{isDetecting && !needsManualInput ? "위치를 찾고 있어요…" : reportLocation?.label ?? "위치를 설정해 주세요"}</span></span>
        </button>
        ...
        <section className="mt-8">
          ...
          <div className="mt-3 grid grid-cols-3 gap-2.5">
            {Array.from({ length: 3 }).map((_, index) => {
              ...
              if (index === files.length) return <button key="add-image" type="button" onClick={() => inputRef.current?.click()} className="flex aspect-square flex-col items-center justify-center rounded-[18px] border border-dashed border-[#bcd3df] bg-white text-[#718594]"><ImagePlus size={23} /><span className="mt-1 text-xs font-bold">사진 추가</span></button>;
          ...
          <input ref={inputRef} type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={addImages} className="hidden" />
        ...
        {mutation.isError && <p className="mt-6 rounded-2xl bg-[#fff3f0] p-4 text-center text-sm font-semibold text-[#b4534a]">제보를 올리지 못했어요. 잠시 후 다시 시도해 주세요.</p>}
      </section>}

      <div className="mobile-fixed">{step === 1 ? <button type="button" disabled={!canGoToStory} onClick={goToStoryStep} className="primary-button">다음 <ChevronRight size={19} /></button> : <button type="submit" disabled={mutation.isPending || !canSubmitReport} className="primary-button">{mutation.isPending ? <><LoaderCircle size={20} className="animate-spin" /> 올리는 중…</> : "제보 올리기"}</button>}</div>
      <LocationPicker open={needsManualInput} current={reportLocation?.label} isDetecting={isDetecting} required={!reportLocation} onClose={() => setNeedsManualInput(false)} onDetect={detectLocation} onSelect={(next) => { setSelectedReportLocation(next); setLocation(next); setNeedsManualInput(false); showToast(`${next.label}으로 위치를 설정했어요.`, "INFO"); }} />
    </form>
  );
}
